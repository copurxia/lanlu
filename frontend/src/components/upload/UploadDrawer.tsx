...
export function UploadDrawer({ open: controlledOpen, onOpenChange, onUploadComplete, trigger }: UploadDrawerProps) {
  ...
  const sortedUploadFiles = useMemo(() => {
    return [...uploadFiles].sort((a, b) => {
      // "fileExists" 需要优先展示，便于用户尽快处理强制覆盖
      if (a.status === "fileExists" && b.status !== "fileExists") {
        return -1
      }
      if (a.status !== "fileExists" && b.status === "fileExists") {
        return 1
      ...
      const aIsCompletedOrError = a.status === "success" || a.status === "error"
      const bIsCompletedOrError = b.status === "success" || b.status === "error"

      if (aIsCompletedOrError !== bIsCompletedOrError) {
        return aIsCompletedOrError ? 1 : -1
      }
  ...
  const uploadStatusCounts = useMemo(() => {
    return uploadFiles.reduce((counts, file) => {
      if (file.status === "success") {
        counts.success += 1
      }
      if (file.status === "error" || file.status === "fileExists") {
        counts.failed += 1
      }
      counts.total += 1
      return counts
  ...
  const sortedDownloadTasks = useMemo(() => {
    return [...downloadTasks].sort((a, b) => {
      const aIsCompletedOrError = a.status === "success" || a.status === "error"
      const bIsCompletedOrError = b.status === "success" || b.status === "error"
  ...
  const forceUpload = (uploadFile: UploadFile) => {
    // 重置状态为排队中
    setUploadFiles(prev => prev.map(f =>
      f.id === uploadFile.id ? { ...f, status: "queued", progress: 0, error: undefined } : f
    ))
  ...
  const hasActiveTransfers = () => {
    const uploadingFiles = uploadFiles.filter(file => file.status === "uploading" || file.status === "queued")
    const downloadingTasks = downloadTasks.filter(task => task.status === "downloading")
    return uploadingFiles.length > 0 || downloadingTasks.length > 0
  }
              ...
              {uploadFiles.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground whitespace-nowrap">
                        （{uploadStatusCounts.success}/{uploadStatusCounts.failed}/{uploadStatusCounts.total}）
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="max-h-[60vh] overflow-y-auto pr-2 space-y-4">
                    {sortedUploadFiles.map((uploadFile) => (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Button
                              ...
                              disabled={uploadFile.status === "uploading" || uploadFile.status === "queued"}
                        ...
                        {(uploadFile.status === "uploading" || uploadFile.status === "queued") && (
                          <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                              <span>{uploadFile.status === "queued" ? "排队中" : t("upload.uploading")}</span>
