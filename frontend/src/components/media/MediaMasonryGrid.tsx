...
const ENABLE_MEDIA_MASONRY_VIRTUALIZATION =
  process.env.NEXT_PUBLIC_HOME_MASONRY_VIRTUALIZATION !== 'false';

type MediaMasonryGridProps = {
  items: Array<Archive | Tankoubon>;
  selectionMode?: boolean;
  selectedArchiveIds?: ReadonlySet<string>;
  selectedTankoubonIds?: ReadonlySet<string>;
...
const MediaTankoubonMasonryCard = memo(function MediaTankoubonMasonryCard({
  index,
...
const MediaArchiveMasonryCard = memo(function MediaArchiveMasonryCard({
  archive,
  index,
  onRequestEnterSelection,
...
export const MediaMasonryGrid = memo(function MediaMasonryGrid({
  items,
  selectionMode = false,
  selectedArchiveIds = EMPTY_SELECTED_IDS,
  selectedTankoubonIds = EMPTY_SELECTED_IDS,
  onRequestEnterSelection,
  onToggleArchiveSelect,
  onToggleTankoubonSelect,
  scrollContainerRef,
  priorityCount = 4,
  virtualized = ENABLE_MEDIA_MASONRY_VIRTUALIZATION,
  ...
  const handleRequestEnterSelection = onRequestEnterSelection ?? (() => {});
  const handleToggleArchiveSelect = onToggleArchiveSelect ?? (() => {});
  const handleToggleTankoubonSelect = onToggleTankoubonSelect ?? (() => {});
  const selectable = Boolean(onRequestEnterSelection);
  ...
  const renderedItems = virtualized ? masonry.visibleItems : masonry.layoutItems;

  return (
    <div ref={containerRef}>
      <div className="relative" style={{ height: `${masonry.totalHeight}px` }}>
        {renderedItems.map((layoutItem) => {
          ...
          if (isTankoubonItem(item)) {
            return (
              <div key={key} ref={bindMeasureRef(key)} className="absolute" style={style}>
                <MediaTankoubonMasonryCard
                  index={index}
                  onRequestEnterSelection={handleRequestEnterSelection}
                  onToggleTankoubonSelect={handleToggleTankoubonSelect}
