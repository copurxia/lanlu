...
import {MetadataAssetsEditor} from './MetadataAssetsEditor';
...
export function ArchiveEditDialog({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await updateArchiveMetadata(archiveId, {
        title,
        type: 0,
        description: summary,
        tags,
  ...
  }, [archiveId, title, summary, tagsText, assetCoverId, assetBackdropId, assetClearlogoId, onSaved, onClose, t]);

  // Handle run metadata plugin
  const handleRunPlugin = useCallback(async () => {
    ...
    try {
      ...
      if (preview) {
        ...
        function applyAssetValue(rawValue: string, setPath: (v: string) => void, setId: (v: string) => void) {
          const trimmed = String(rawValue || '').trim();
          if (!trimmed) return;
          if (/^\d+$/.test(trimmed)) {
            const parsedId = Number.parseInt(trimmed, 10);
            if (Number.isFinite(parsedId) && parsedId > 0) {
              setId(String(parsedId));
              setPath('');
              return;
            }
          }
        ...
        applyAssetValue(preview.cover, () => {}, setAssetCoverId);
        applyAssetValue(preview.backdrop, () => {}, setAssetBackdropId);
        applyAssetValue(preview.clearlogo, () => {}, setAssetClearlogoId);
      }
        ...
        <View style={[styles.overlay, {paddingTop: 0}]}>
          <View style={styles.container}>
            ...
            <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
              ...
              {/* Title */}
              <Text style={styles.fieldLabel}>{t('archive.titleField')}</Text>
              <TextInput
                style={styles.input}
                value={title}
              ...
              {/* Summary */}
              <Text style={styles.fieldLabel}>{t('archive.summary')}</Text>
              <TextInput
              ...
              {/* Tags */}
              <Text style={styles.fieldLabel}>{t('archive.tags')}</Text>
              <TextInput
                style={styles.input}
                value={tagsText}
