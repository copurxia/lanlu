...
export type ChannelPreviewLayout<T extends ChannelPreviewLayoutItem> =
  | {kind: 'single'; hero: T; heroHeight: number}
  | {kind: 'rows'; rows: ChannelPreviewRow<T>[]}
  | {kind: 'hero-top'; hero: T; heroHeight: number; rows: ChannelPreviewRow<T>[]}
  | {kind: 'hero-side'; hero: T; heroWidth: number; totalHeight: number; rows: ChannelPreviewRow<T>[]};
...
function clampAspectRatio(aspectRatio: number): number {
  if (!Number.isFinite(aspectRatio) || aspectRatio <= 0) return DEFAULT_CHANNEL_ASPECT_RATIO;
...
function normalizeRowItems<T extends ChannelPreviewLayoutItem>(
  ...
  const availableWidth = Math.max(0, rowWidth - CHANNEL_ROW_GAP * Math.max(0, row.length - 1));
  const rawWidths = row.map(item => height * item.aspectRatio);
  const residual = availableWidth - rawWidths.reduce((sum, width) => sum + width, 0);
  const adjustedWidths = rawWidths.map((width, index) => (index === rawWidths.length - 1 ? width + residual : width));
  return row.map((item, index) => ({...item, width: adjustedWidths[index]}));
}
...
function buildJustifiedRows<T extends ChannelPreviewLayoutItem>(
  ...
  if (sourceItems.length === 1) {
    ...
    return [{height: width / Math.max(sourceItems[0].aspectRatio, 0.1), items: [{...sourceItems[0], width}]}];
  }

  const normalizedRowCount = Math.max(1, Math.min(rowCount, sourceItems.length));
  const targetRowHeight = Math.max(
    MIN_ROW_HEIGHT,
    (maxTotalHeight - CHANNEL_ROW_GAP * Math.max(0, normalizedRowCount - 1)) / normalizedRowCount,
  );
  ...
  return rows.map(row => {
    ...
    const rowGapWidth = CHANNEL_ROW_GAP * Math.max(0, row.length - 1);
    const height = (rowWidth - rowGapWidth) / ratioSum;
    return {height, items: normalizeRowItems(row, rowWidth, height)};
  });
}

function measureRowsHeight<T extends ChannelPreviewLayoutItem>(rows: ChannelPreviewRow<T>[]): number {
  return rows.reduce((sum, row) => sum + row.height, 0) + CHANNEL_ROW_GAP * Math.max(0, rows.length - 1);
}
...
function selectRows<T extends ChannelPreviewLayoutItem>(
  ...
  if (sourceItems.length === 0) return {rowCount: 0, rows: [], totalHeight: 0};
  ...
  for (let rowCount = 1; rowCount <= maxRowCount; rowCount += 1) {
    ...
    if (totalHeight > maxTotalHeight + HEIGHT_TOLERANCE) continue;
    const score = Math.abs(preferredRowCount - rowCount) * 48 + Math.abs(maxTotalHeight - totalHeight);
    if (score < bestScore) {
      bestScore = score;
      best = {rowCount, rows, totalHeight};
    }
  }
  return best;
}
...
function chooseHeroPlacement<T extends ChannelPreviewLayoutItem>(sourceItems: T[]) {
  ...
  return {heroIndex: preferSide ? sideHeroIndex : topHeroIndex, placement: preferSide ? 'side' as const : 'top' as const};
}
...
function solveHeroSideLayout<T extends ChannelPreviewLayoutItem>(
  ...
  const maxHeightByWidth = Math.min(
    ...
    (containerWidth - CHANNEL_ROW_GAP - MIN_SIDE_RIGHT_WIDTH) / Math.max(hero.aspectRatio, 0.1),
  );
  if (!Number.isFinite(maxHeightByWidth) || maxHeightByWidth <= MIN_TOP_HERO_HEIGHT) return null;

  let best: {heroWidth: number; rows: ChannelPreviewRow<T>[]; totalHeight: number} | null = null;
  let bestScore = Number.POSITIVE_INFINITY;

  for (let rowCount = 1; rowCount <= maxRowCount; rowCount += 1) {
    const evaluate = (candidateHeight: number) => {
      const heroWidth = candidateHeight * hero.aspectRatio;
      const rightWidth = containerWidth - heroWidth - CHANNEL_ROW_GAP;
      ...
      return {heroWidth, rows, totalHeight: measureRowsHeight(rows)};
    };

    const upperBound = Math.min(CHANNEL_MAX_COLLAGE_HEIGHT, maxHeightByWidth);
    const maxEvaluation = evaluate(upperBound);
    if (!maxEvaluation || maxEvaluation.totalHeight > upperBound + HEIGHT_TOLERANCE) continue;
    ...
    for (let step = 0; step < BINARY_SEARCH_STEPS; step += 1) {
      ...
      if (Math.abs(diff) <= HEIGHT_TOLERANCE) break;
      if (diff > 0) low = candidateHeight;
      else high = candidateHeight;
    }

    if (resolvedEvaluation.totalHeight > CHANNEL_MAX_COLLAGE_HEIGHT + HEIGHT_TOLERANCE) continue;
    const score = Math.abs(desiredRowCount - rowCount) * 48 + Math.abs(CHANNEL_MAX_COLLAGE_HEIGHT - resolvedEvaluation.totalHeight);
    if (score < bestScore) {
      bestScore = score;
      best = resolvedEvaluation;
    }
  }

  return best ? {kind: 'hero-side', hero, heroWidth: best.heroWidth, totalHeight: best.totalHeight, rows: best.rows} : null;
}
...
function buildHeroTopLayout<T extends ChannelPreviewLayoutItem>(
  ...
  const maxHeroHeight = Math.max(
    MIN_TOP_HERO_HEIGHT,
    CHANNEL_MAX_COLLAGE_HEIGHT - (restItems.length > 0 ? CHANNEL_ROW_GAP : 0) - minRowsHeight,
  );
  const heroHeight = Math.max(MIN_TOP_HERO_HEIGHT, Math.min(preferredHeroHeight, Math.max(MIN_TOP_HERO_HEIGHT, maxHeroHeight)));
  const remainingHeight = Math.max(0, CHANNEL_MAX_COLLAGE_HEIGHT - heroHeight - (restItems.length > 0 ? CHANNEL_ROW_GAP : 0));
  const preferredRowCount = restItems.length <= 2 ? 1 : restItems.length <= 4 ? 2 : 3;
  const rowSelection = selectRows(restItems, containerWidth, preferredRowCount, remainingHeight) || {rowCount: 0, rows: [], totalHeight: 0};
  return {kind: 'hero-top', hero, heroHeight, rows: rowSelection.rows};
}
...
export function computeChannelPreviewLayout<T extends ChannelPreviewLayoutItem>(
  ...
  const normalizedItems = items.map(item => ({...item, aspectRatio: clampAspectRatio(item.aspectRatio)}));
  const effectiveWidth = Math.max(containerWidth, 320);

  if (normalizedItems.length === 0) return {kind: 'rows', rows: []};
  if (normalizedItems.length === 1) {
    return {kind: 'single', hero: normalizedItems[0], heroHeight: Math.min(360, effectiveWidth / Math.max(normalizedItems[0].aspectRatio, 0.1))};
  }
  if (normalizedItems.length === 2) return {kind: 'rows', rows: buildJustifiedRows(normalizedItems, effectiveWidth, 1, 260)};
