...
[CangjieGUI](../ref/CangjieGUI)（cui，本地路径依赖）。覆盖 v1 范围：登录、书库浏览
...
| SDL3 动态库 | 位于 `../ref/CangjieGUI/sdl/.sdl3/`，运行时必须加入 `LD_LIBRARY_PATH` |
...
```bash
cd client-next
LD_LIBRARY_PATH=../ref/CangjieGUI/sdl/.sdl3:$LD_LIBRARY_PATH ./target/release/bin/main
```
...
```bash
SDL_VIDEODRIVER=wayland LD_LIBRARY_PATH=../ref/CangjieGUI/sdl/.sdl3:$LD_LIBRARY_PATH \
...
```bash
cd client-next
SDL_VIDEODRIVER=dummy LD_LIBRARY_PATH=../ref/CangjieGUI/sdl/.sdl3:$LD_LIBRARY_PATH \
