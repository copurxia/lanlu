...
const STATUS_CACHE_TTL_MS = 24 * 60 * 60 * 1000 // 24 小时
...
function isCacheEntryValid(entry: TabStatusCacheEntry | undefined): boolean {
  if (!entry) return false
  return Date.now() - entry.updatedAt < STATUS_CACHE_TTL_MS
}
...
export default function AddPage({ navigate }: AddPageProps) {
    ...
    await new Promise((resolve) => {
      chrome.storage.local.get(STATUS_CACHE_KEY, (items) => {
        ...
        for (const key of Object.keys(next)) {
          if (now - (next[key]?.updatedAt ?? 0) >= STATUS_CACHE_TTL_MS) {
            delete next[key]
          }
        }
  ...
  const runCurrentCheck = useCallback(async (forceRefresh = false) => {
    if (!auth) return
    ...
    try {
      ...
      if (!forceRefresh) {
        ...
        if (isCacheEntryValid(cached) && cached?.status === "saved" && cached.arcid) {
      ...
      <div className="rounded-lg border bg-card text-card-foreground p-3 space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            onClick={() => void runCurrentCheck(true)}
